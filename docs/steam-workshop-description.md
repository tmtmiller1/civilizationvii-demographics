...
[*]Spans the full metric set: score, economy, military power, population, knowledge, influence, resources, and more, so you can quickly compare leaders across each metric.
...
[*]Each war reports its cost as the observed change in the participants' military, settlements, population, and production across the war, computed directly from recorded per-turn samples without inferred values.
...
[*]A per-war Graphs view plots those same metrics over just that war's window for just its belligerents, so you can see when a side's strength shifted.
[/list]
