...
[/list]
[h2]Languages[/h2]
Fully localized in English, German, Spanish, French, Italian, Japanese, Korean, Portuguese (Brazil), Russian and Simplified Chinese, using the game's own number formatting per language. Polish is included and in progress.
[h2]Notes[/h2]
Read-only and additive: it does not change balance, alter opponents, or overwrite base-game files. Per-save data storage; the Hall of Fame is kept on your computer.
[h2]A Civilization VII bug, and the workaround for it[/h2]
Civilization VII 1.5.0 has a bug in the storage mods use for their settings: whenever a mod asks for its own saved data, the game returns whichever mod's entry sorts first. Only that one mod's data works; the entry most mod option panels share can never be read back, so their settings reset every launch and the Hall of Fame cannot save its list of past games (every game is still stored in its own save). It is Firaxis's bug to fix; a reproduction is written up for them. When it blocks the Hall of Fame, a notice box on every Hall of Fame page names the bug, and clicking it opens [b]Repair mod storage[/b], a sheet that explains the bug and the button before you can press anything. The button empties the storage and leaves one entry in it, the shared settings entry used by most mods with an options panel, holding Demographics' settings and past game information; being the only entry, it is what the game returns, so the mods that use it save settings normally from then on. It deletes every separate entry, including the one that was sorting first and was therefore the only one actually working. Saves, the history inside them, the game's settings and every mod's files are untouched. Two clicks, with a Cancel. Full explanation: https://github.com/tmtmiller1/civilizationvii-demographics/blob/main/docs/civ7-storage-bug.md
