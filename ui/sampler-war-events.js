...
function cityLandKm2(cityID) {
  try {
    ...
    if (city && typeof city.getPurchasedPlots === "function") {
      const plots = city.getPurchasedPlots();
      const n = Array.isArray(plots) ? plots.length : plots?.length;
      if (typeof n === "number" && isFinite(n)) return n * LAND_KM2_PER_TILE;
    }
  } catch (_) {
