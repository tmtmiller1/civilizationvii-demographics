...
// from chart-line.js (remediation #26). None of these own any shared state -
...
export function makeCapLimitLinePlugin(metricId) {
  return {
    ...
    afterDatasetsDraw(c) {
      ...
      ctx2.font = "11px " + resolveChartFontFamily(c);
      const text = t("LOC_DEMOGRAPHICS_LINE_CAP_LIMIT");
      const textW = ctx2.measureText(text).width;
      const labelX = Math.max(left + 2, right - textW - 8);
      const labelY = Math.max(c.chartArea.top + 2, y - 14);
