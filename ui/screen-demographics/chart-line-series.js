...
// no Chart.js. Extracted from chart-line.js (remediation #26).
 ...
 * @param {boolean} fromContactOnly When true, withhold points from samples
 *   taken before the local player met this civ (met === false), so the line
 *   begins at first contact. When false (back-fill mode), all points are kept
 *   and the caller gates the whole civ on its current met status instead.
 ...
 *   the whole civ.
 */
function foldPidSamples(samples, pid, metricId, ageOffsets, ageBoundariesLocal, fromContactOnly) {
  /** @type {PidFold} */
  const fold = {
    points: [],
    leaderType: null,
...
function buildSeriesEntry(samples, pid, idx, fold, eliminatedMap) {
  const { points, leaderType, leaderTypeString, leaderName, primaryColor, secondaryColor, metFlag } =
    fold;
  ...
  const color =
    typeof primaryColor === "string" && primaryColor.length > 0
      ? preferReadableColor(primaryColor, secondaryColor)
...
function buildSeriesFromHistory(history, metricId) {
  ...
  pids.forEach((pid, idx) => {
    const fold = foldPidSamples(
      samples, pid, metricId, ageOffsets, ageBoundariesLocal, fromContactOnly
    );
