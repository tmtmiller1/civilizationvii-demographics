...
import { scaleCityPopulationAt } from "/demographics/ui/demographics-metrics.js";
import {
  getFounded,
  getCityTrend
} from "/demographics/ui/screen-demographics/settlements-trace.js";
import { preferReadableColor, safeTextColor } from "/demographics/ui/civ-color-utils.js";
 ...
 * @property {boolean} [met] Whether the LOCAL player has met this owner (undefined when unreadable).
 */
 ...
 * @property {Array<{type: string, icon: string, nameKey: string, location: {x: number, y: number}|null}>} [wonders] Completed wonders (confirmed).
 * @property {{turn: number, year: string, exact: boolean}|null} [founded] Founding (exact via event, else approx).
 * @property {{popGrowthPerTurn: number, dir: number, samples: number}|null} [trend] Population trend.
 ...
 * @type {Array<{ id: string, yt: string|null, label: string, icon: string, composite: boolean, weight: number }>}
 */
...
function localHasMet(pid) {
  try {
    const localId = typeof GameContext !== "undefined" ? GameContext.localPlayerID : undefined;
    if (typeof localId !== "number") return undefined;
    if (pid === localId) return true;
    const localP = typeof Players !== "undefined" && Players.get ? Players.get(localId) : null;
 ...
 * @param {*} city The city handle.
 * @returns {Array<{type: string, icon: string, nameKey: string, location: {x: number, y: number}|null}>} The wonders.
 */
function readWonderList(city) {
  /** @type {Array<{type: string, icon: string, nameKey: string, location: {x: number, y: number}|null}>} */
  const out = [];
  const con = city?.Constructibles;
 ...
 * @returns {{type: string, icon: string, nameKey: string, location: {x: number, y: number}|null}|null}
 */
function resolveWonder(cid) {
  try {
    const c = typeof Constructibles !== "undefined" ? Constructibles.getByComponentID(cid) : null;
    if (!c || !c.complete) return null;
    const table = gameInfoTable("Constructibles");
    const info = table && typeof table.lookup === "function" ? table.lookup(c.type) : null;
    const type = info && info.ConstructibleType;
    if (typeof type !== "string") return null;
    return { type, icon: wonderIconUrl(type), nameKey: info.Name || type, location: readLocation(c) };
  } catch (_) {
    return null;
  }
}
 ...
 * @property {Record<string, Settlement|null>} leaders Output id (and "composite") → its rank-1 settlement.
 */
