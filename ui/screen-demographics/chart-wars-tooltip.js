...
import {
  buildCostTable,
  buildVsCell
} from "/demographics/ui/screen-demographics/chart-wars-cost-table.js";
import {
  majorsOnSide,
  warDurationYears
...
export function renderWarTooltip(tooltip, w, ctx) {
  ...
  // Centered header (game tile-tooltip style): title, a timeline subtitle, a
  // status pill, then a muted declarer line.
  ...
  // City-state allies: their own table BELOW a divider, same column-with-labels
  // layout as the majors (combined column + one per ally), Military Power rows.
 ...
 * @param {Snapshot[]} win The war-window samples.
 * @param {*} tip The tooltip body (for the war window).
 */
function pushSideCols(cols, majors, csList, samples, win, tip) {
  const csTotals = csList.length ? csSideTotals(csList, win) : null;
  for (const entry of majors) {
    cols.push({ entry, cs: csTotals, cost: participantCost(samples, entry, tip.warStart, tip.warEnd) });
  }
}
...
function buildSidesEl(tip, samples, win) {
  /** @type {*[]} */
  const cols = [];
  pushSideCols(cols, tip.sideA, tip.csA, samples, win, tip);
  const aCount = cols.length;
  pushSideCols(cols, tip.sideB, tip.csB, samples, win, tip);
