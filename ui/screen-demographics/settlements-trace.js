 ...
 * @returns {{seed: string, founded: Record<string, *>, pop: Record<string, Array<{t: number, pop: number}>>}}
 */
...
function onCityAdded(data) {
  safe(() => {
    const cid = data && (data.cityID || data.city);
    const city = cid && typeof Cities !== "undefined" && Cities.get ? Cities.get(cid) : null;
...
/**
 * Record this sample's settlement populations into the rolling window, and stamp
 * an APPROXIMATE founding for any settlement not yet recorded (and fill the year
 * for an exact-but-yearless founding stamped between samples).
...
export function recordSettlementsNow(turn, year) {
  ...
  safe(() => DemographicsSettings.setSetting(KEY, blob));
}
...
export function getCityTrend(locId) {
  if (!locId) return null;
  const blob = DemographicsSettings.getSetting(KEY, null);
  if (!blob || blob.seed !== gameSeed() || !blob.pop) return null;
  ...
  return { popGrowthPerTurn: rate, dir: rate > 0.001 ? 1 : rate < -0.001 ? -1 : 0, samples: arr.length };
}
