 ...
 * @param {{ padL:number, padT:number, innerW:number, innerH:number, yOf:(v:number)=>number }} L Chart layout.
 * @param {number} yMax Maximum y-domain value.
 * @param {StackGridConfig} cfg Grid configuration.
 * @param {(tag:string, attrs:Record<string, string|number>) => SVGElement} svgEl SVG builder helper.
 */
 ...
 * @param {SVGElement} svg Chart SVG element.
 * @param {{ padT:number, innerH:number, xOf:(t:number)=>number }} L Chart layout.
 * @param {{ xMin:number, xMax:number }} dom X-domain.
 * @param {Map<number, string>} turnYearMap Turn-to-year map.
 * @param {StackGridConfig} cfg Grid configuration.
 * @param {(turnMap:Map<number, string>, t:number)=>string|null} nearestByTurn Year resolver helper.
 * @param {(tag:string, attrs:Record<string, string|number>) => SVGElement} svgEl SVG builder helper.
 * @returns {{ t:number, x:number, year:string|null, labelY:number }[]} Tick positions.
 */
export function drawStackXTicks(svg, L, dom, turnYearMap, cfg, nearestByTurn, svgEl) {
 ...
 * @param {{ W:number, H:number, mode:"turn"|"year"|"both", className:string, turnParenWhenBoth?:boolean }} opts Render options.
 */
 ...
 * @param {HTMLElement} wrap Chart wrap element.
 * @param {{ L:{padL:number,innerW:number,padT:number,innerH:number}, W:number, H:number, xClassName:string, yClassName:string, xText:string, yText:string }} opts Axis render options.
 */
