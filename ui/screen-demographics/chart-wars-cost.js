 ...
 * @property {number | null} milLost Combat strength lost: true casualties (increase in cumulative units-killed strength) over the window, or the standing-army decline proxy for pre-tracking saves.
 * @property {number | null} settlementsChange Net cities won/lost by capture (event-based; +captured / -lost; founding excluded; null pre-tracking).
 * @property {number | null} razed Settlements permanently razed (accrued; null for pre-tracking wars).
 * @property {number | null} landChange Net territory change in km² (signed: +gained / -ceded).
 * @property {number | null} popLost Gross population lost (sum of every per-turn decline; growth never masks it).
 * @property {number | null} cropLost Crop yield (food/turn) lost: sum of every per-turn decline.
 * @property {number | null} prodLost Production lost: sum of every per-turn drop in production-per-turn.
 * @property {number | null} warProd Production directed to war (military units + buildings), accrued over the window.
 */
 ...
 * @type {{ id: string, key: string, mode: "drawdown" | "net" | "losses" | "accrued" | "spent" | "level",
 *   label: string, blp: string, glossary: string, series?: string }[]}
 */
