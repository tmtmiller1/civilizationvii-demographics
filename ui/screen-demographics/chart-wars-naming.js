...
import { dlog, buildStackTurnYears } from "/demographics/ui/screen-demographics/chart-shared.js";
...
export function buildWarNameOverrides(filtered, turnYearMap, latestTurn, continentMap) {
  ...
  // Duration-flair calibration, measured in TURNS (how long the war dragged on
  // from the player's seat), NOT in-game years: Civ's timeline compresses
  // unevenly, so a 3-turn Antiquity war spans 100+ years while a 15-turn Modern
  // war spans ~30 - the old year test both over-fired AND inverted that. Cutoffs
  // are RELATIVE to this game's typical war length (median) so the flair stays
  // rare across speeds / eras, with an absolute floor so an all-brief-wars game
  // labels nothing. Calibrated over the same (filtered) set as the ordinals.
  ...
  // Rare by design: the vast majority of wars get NO suffix. Civ VII blocks a
  // negotiated peace before turn 10, so a "normal" war clusters around ~10 turns
  // (peace is made as soon as it's allowed); shorter ones come only from
  // eliminations/capitulations. A war earns the evocative duration name only when
  // it ran at least DOUBLE that floor (>= 20 turns - i.e. it kept fighting ~10+
  // turns AFTER peace was first available) AND at least 2x this game's median war
  // (so it scales with game pace and stays rare). Brief wars never qualify, and
  // there is no generic "(Long War)" tag any more.
