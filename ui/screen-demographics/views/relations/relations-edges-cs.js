...
function relationshipKeyMap() {
  ...
  map.set(relEnum.PLAYER_RELATIONSHIP_ALLIANCE, "alliance");
  map.set(relEnum.PLAYER_RELATIONSHIP_FRIENDLY, "friendly");
  map.set(relEnum.PLAYER_RELATIONSHIP_HELPFUL, "helpful");
  map.set(relEnum.PLAYER_RELATIONSHIP_NEUTRAL, "neutral");
  map.set(relEnum.PLAYER_RELATIONSHIP_UNFRIENDLY, "unfriendly");
  map.set(relEnum.PLAYER_RELATIONSHIP_HOSTILE, "hostile");
  map.set(relEnum.PLAYER_RELATIONSHIP_AT_WAR, "war");
  return map;
}
...
/**
 * Whether the city-state bonus lookup surface is available.
 * @returns {boolean}
 */
function canResolveCsBonusType() {
  if (!Game?.CityStates) return false;
  if (typeof Game.CityStates.getBonusType !== "function") return false;
  if (typeof GameInfo === "undefined") return false;
  return !!GameInfo.CityStateBonuses;
}
...
function csTypeFromBonus(pid) {
  if (!canResolveCsBonusType()) return null;
  const bonusHash = Game.CityStates.getBonusType(pid);
  if (bonusHash == null || bonusHash === -1) return null;
...
/**
 * Whether the independent-city-state lookup surface is available.
 * @returns {boolean}
 */
function canResolveIndependentCsType() {
  if (typeof Players?.get !== "function") return false;
  if (typeof GameInfo === "undefined") return false;
  return !!GameInfo.Independents;
}
...
function csTypeFromIndependents(pid) {
  if (!canResolveIndependentCsType()) return null;
  const player = Players.get(pid);
  const adj = player?.civilizationAdjective;
  if (typeof adj !== "string" || adj.length === 0) return null;
