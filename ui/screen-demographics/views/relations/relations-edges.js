...
export {
  ...
  csTypeMeta,
  resolveCsType
} from "/demographics/ui/screen-demographics/views/relations/relations-edges-cs.js";
...
function actionPairFromEvent(ev, sourcePid, actionType, metSet) {
  if (!ev || ev.actionType !== actionType) return null;
  const other = resolveEventOther(ev, sourcePid);
  if (typeof other !== "number" || other === sourcePid) return null;
  if (!metSet.has(other)) return null;
  return { a: sourcePid, b: other };
}
...
function buildActionTypeEdges(metIds, actionType, color, filterKey) {
  ...
  for (const a of metIds) {
    ...
    for (const ev of events) {
      const pair = actionPairFromEvent(ev, a, actionType, metSet);
      if (!pair) continue;
...
function pushEndeavorEdges(actionList, metIds, filterKey) {
  ...
  for (const a of metIds) {
    for (const ev of getPlayerEvents(a)) {
      pushOneEndeavorEdge(edges, seen, ev, a, ctx);
    }
  }
  return edges;
}
...
export function buildEconomicEdges(metIds, _filterKey, localPid) {
  ...
  for (const fromPid of pids) {
    ...
    if (!trade) continue;
    pushTradeEdgesForSource(edges, trade, fromPid, pids);
  }
  return edges;
}
