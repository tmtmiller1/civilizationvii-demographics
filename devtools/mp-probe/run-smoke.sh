#!/bin/zsh
# run-smoke.sh - hands-free hardening smoke run (hotseat) using smoke-game.js as the game driver.
...
say "backup -> $BAK"; LS="$S/LocalStorage.sqlite"; T=$(sqlite3 "$LS" "select name from sqlite_master where type='table' limit 1"); sqlite3 "$LS" "select key, length(value) from \"$T\"" > "$RUN/localstorage-before.txt" 2>/dev/null || true
...
mkdir -p "$DEST"
cp "$HERE/demographics-mp-probe.modinfo" "$DEST/"; cp "$HERE/smoke-shell.js" "$DEST/hs-shell.js"; cp "$HERE/smoke-game.js" "$DEST/hs-game.js"
: > "$LOG" 2>/dev/null
...
sqlite3 "$LS" "select key, length(value) from \"$T\"" > "$RUN/localstorage-after.txt" 2>/dev/null || true; cleanup
say "FINISHED -> $RUN"
