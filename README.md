...
- **Your view, your way:** per-civ colors, time-range filters, hide and focus, civ-or-leader labels, colorblind mode, CSV export, a font-size setting, and a Basic mode that shows only the core pages.
- **Fits your display:** the dashboard scales to the resolution it is played at, from 1280x720 to 4K, rather than being laid out for one screen size.
...
<table>
  ...
  <tr><td width="50%" align="center" valign="top"><a href="docs/screenshots/38-cinematic-flyby.jpg"><img src="docs/screenshots/38-cinematic-flyby.jpg" alt="The cinematic flyby orbiting the world's greatest settlement, with its info panel at the bottom of the screen"></a><br><sub>Cinematic view: the camera tours a settlement's wonders and quarters</sub></td><td width="50%" align="center" valign="top"></td></tr>
</table>
...
Every chart page shares one toolbar: a time-range filter (25 to 1000 years, current age, a single age, or all time), a **Civ → Leader** / **Leader → Civ** label order, a time-axis mode, wonder markers on or off, and **Copy as CSV**. Charts mark each crisis stage and age change as a vertical line, and wonders as icons on the line of the civilization that built them. The legend toggles civilizations on and off.

The **Options** button and the active analytics policy sit together at the top right of the screen, level with the title, on every tab.
...
The dashboard is drawn as one uniformly scaled frame against a 2880x1800 reference, so the same layout reaches every display rather than filling the screen at one resolution and crowding it at another. The scale has a lower bound and text has its own, higher one, so nothing shrinks past reading size. 2880x1800 renders at full size; 3840x2160, 2560x1440, 1920x1080, 1600x900 and 1366x768 approximate it. At 1280x720, the smallest supported resolution, the layout has reached its bound while the text has not, so some pages are tighter than intended — everything is reachable and readable, but column headings and long leader names can be cut off. The **Font Size** setting (game Options → Mods → Demographics) shifts the whole type scale up or down from there.
...
- The **Options** button, at the top right of every tab, opens the game's Options screen at **Mods → Demographics**.
- **Escape** closes the dashboard, as it closes the Hall of Fame.

## 12. Compatibility

- Does not overwrite base-game files.
- All engine reads are defensive: schema drift yields missing data, not crashes. A view that fails says "Chart failed to render" in its own panel, and the rest of the screen keeps working.
- The mod's CSS custom properties are namespaced (`--dg-ia-`), so it styles only its own screen and leaves other mods' variables alone. Every write to the shared settings entry is read back and confirmed before the next one, so Demographics keeps its own slice and leaves the rest of that entry as it found it.
